...
tailDependence <- function(formula, data, q, tail = "lwr", method = "dabrowska"){
    ...
    if(method == "dabrowska"){
        ...
        Fx <- 1 - KMx
        xuni2 <- sort_unique(xx)
        ifelse(tail == "lwr", out <- c(Fx[match(1, xuni2 >= qq)], out), out <- c(KMx[match(1, xuni2 >= qq)], out))
    }
    if(out[1] < 0 | out[1] > 1){
...
tailDep <- function(formula, data, q, tail = "lwr", method = "fast"){
    ...
    if(method == "dabrowska"){
        ...
        Fx <- 1 - KMx
        xuni2 <- sort_unique(xx)
        ifelse(tail == "lwr", out <- Fx[match(1, xuni2 >= qq)], out <- KMx[match(1, xuni2 >= qq)])
    }
    if(out < 0 | out > 1){
        stop("q is too close to either 0 or 1")
    }
